...
passive nodes. Here we assumed that $\delta = 3$. Notice also that each
configuration is a set of labels that are allowed to be on edges 
incident to any active/passive node $v$. Notice that the order of
the labels in a single configuration does not matter. Next, we will
